...
class Metrics(object):
    ...
    def report_confusion_matrix(self, name, _path='img'):
        """计算混淆矩阵"""
        ...
        # 保存热力图, 上面的计算方式亦可以直接用下面api解决
        from sklearn.metrics import confusion_matrix
        import matplotlib.pyplot as plt
        plt.figure(figsize=(12, 10))
        cm = confusion_matrix(self.golden_tags, self.predict_tags, labels=tag_list)
        ht = sn.heatmap(cm, cmap="RdBu_r", xticklabels=tag_list, yticklabels=tag_list)
        fig = ht.get_figure()
        fig.savefig(os.path.join(_path, name), dpi=240)
        fig.clf()
